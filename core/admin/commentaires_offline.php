...
<form action="commentaires_offline.php<?php echo !empty($_GET['a'])?'?a='.$_GET['a']:'' ?>" method="post" id="frm_coms">
<table class="table">
<thead>
	<tr>
...
if($plxAdmin->plxGlob_coms->count AND !empty($plxAdmin->plxRecord_coms->size)) { # On a des commentaires
	...
	<tr>
		<td colspan="5">
			<?php plxUtils::printSelect('selection', array(''=> 'Pour la s&eacute;lection...', 'delete' => 'Supprimer', 'online' => 'Valider'), ''); ?>
...
if($plxAdmin->plxGlob_coms->count) { # Si on a des commentaires (hors page)
	...
	# Generation des URLs
	$p_url = 'commentaires_offline.php?page='.$prev_page.(!empty($_GET['a'])?'&amp;a='.$_GET['a']:''); # Page precedente
	$n_url = 'commentaires_offline.php?page='.$next_page.(!empty($_GET['a'])?'&amp;a='.$_GET['a']:''); # Page suivante
	$l_url = 'commentaires_offline.php?page='.$last_page.(!empty($_GET['a'])?'&amp;a='.$_GET['a']:''); # Derniere page
	$f_url = 'commentaires_offline.php?page=1'.(!empty($_GET['a'])?'&amp;a='.$_GET['a']:''); # Premiere page
