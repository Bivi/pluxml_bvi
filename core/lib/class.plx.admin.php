...
';
	...
	public function editArticle($content, &$id) {
		...
		$xml .= "\t".'<meta_description><![CDATA['.plxUtils::cdataCheck(trim($content['meta_description'])).']]></meta_description>'."\n";
		$xml .= "\t".'<meta_keywords><![CDATA['.plxUtils::cdataCheck(trim($content['meta_keywords'])).']]></meta_keywords>'."\n";
		# Hook plugins
	...
	public function checkMaj() {
		...
		if(version_compare($this->version, $latest_version, ">="))
			return L_PLUXML_UPTODATE.' ('.$this->version.')';
		 	...
		 	return L_PLUXML_UPDATE_AVAILABLE.' <a href="http://pluxml.org/">PluXml.org</a>';
	}
